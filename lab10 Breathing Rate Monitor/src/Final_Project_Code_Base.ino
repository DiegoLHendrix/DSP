...
void loop() {
  ...
  float readValue,
      floatOutput; //  Input data from ADC after dither averaging or from MATLAB
  ...
  numValues =
      2; // The number of columns to be sent to the serial monitor (or MATLAB)

  WriteToSerial(numValues,
                printArray); //  Write to the serial monitor (or MATLAB)
...
int FIR_Generic(long inputX, int sampleNumber) {
  ...
  for (i = 0; i < MFILT; i++) {
    ...
    // Explicitly cast the impulse value and the input value to LONGs then
    // multiply by the input value.  Sum up the output values
  ...
  // Skip the first MFILT  samples to avoid the transient at the beginning due
  // to end effects
...
float IIR_Generic(float xv) {
  ...
  for (i = 0; i < numStages; i++) {
    ...
    yv = -a[i][2] * yM2[i] - a[i][1] * yM1[i] + b[i][2] * xM2[i] +
         b[i][1] * xM1[i] + b[i][0] * xM0[i];
    yM0[i] = yv;
    xv = yv;
  }
  //
