...
int AlarmCheck(float stdLF, float stdBF, float stdHF) {
    float signalThreshold = 400.0;
    if (stdLF < signalThreshold && stdBF < signalThreshold && stdHF < signalThreshold)
        return 1;  // Disconnect
...
int NoiseFilter(long xInput, int sampleNumber) {
    /* Using the FIR_Designer.m file
    *
    * FIR_Designer('nOrder', 61, 'cutBPM', 70, 'Type', 'HPF')
    */
